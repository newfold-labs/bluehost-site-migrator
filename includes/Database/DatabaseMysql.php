 ...
 * Uses the deprecated mysql methods.
 */
		...
		if ( ! ( $result = \mysql_query( $input, $this->wpdb->dbh ) ) ) {
			...
			if ( empty( $this->wpdb->dbh ) || 2006 === $mysql_errno ) {
				if ( ! $this->wpdb->check_connection( false ) ) {
					throw new \Exception( __( 'Error reconnecting to the database. <a href="https://help.servmask.com/knowledgebase/mysql-error-reconnecting/" target="_blank">Technical details</a>', AI1WM_PLUGIN_NAME ), 503 );
				}
